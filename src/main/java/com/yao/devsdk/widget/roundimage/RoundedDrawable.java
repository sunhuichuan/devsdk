package com.yao.devsdk.widget.roundimage;
...
public class RoundedDrawable extends Drawable {
    ...
    /**
     * 外围的白边，向内的比率
     * 等于2，代表有一半是内陷到imageView中的
     */
    private static final int BORDER_INSET_RATIO = 2;

    /**
     * 边缘压着图片内陷
     */
    final boolean IS_BORDER_INSET = false;
    ...
    private void updateShaderMatrix() {
        ...
        float borderInset = (mBorderWidth) / BORDER_INSET_RATIO;
        switch (mScaleType) {
            case CENTER:
                mBorderRect.set(mBounds);
                mBorderRect.inset(borderInset, borderInset);
        ...
        if (!IS_BORDER_INSET){
            //不内陷
            RectF drawableRect = new RectF(mBorderRect.left+borderInset,mBorderRect.top+borderInset,mBorderRect.right-borderInset,mBorderRect.bottom-borderInset);
            mDrawableRect.set(drawableRect);
        }else{
