        ...
        switch (method) {
            case "POST": {
                ...
                if (body instanceof FormBody) {
                    ...
                    if (SdkConst.DEBUG){
                        LoggerUtil.i(TAG,"请求的url为："+original.url().toString());
                        for (int i=0,size=formBody.size();i<size;i++){
                            LoggerUtil.i(TAG,"参数为："+formBody.name(i)+"="+formBody.value(i));
                        }
                    ...
                    for (int i = 0, size = oldFormBody.size(); i < size; i++) {
                        ...
                        if (ReflectionUtil.isInstance(bodyValue, StringRequestBody.class)) {
                            ...
                            LoggerUtil.i(TAG, "name是：" + name + ",content是:" + content);
...
//                        newFormBodyBuilder.addPart(formData);
        ...
        if (SdkConst.DEBUG) {
            LoggerUtil.i(TAG, "当前网络请求线程是：" + Thread.currentThread().getId());
            LoggerUtil.i(TAG, "请求的URL为：" + newRequest.url().toString());
        }
